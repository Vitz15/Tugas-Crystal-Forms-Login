...
namespace Tugas_Crystal_Forms_Login
{
    public partial class FrmMain : Form
    {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtID.Text != "")
                {
                    ...
                    {
                        query = string.Format("Delete from tbl_user where id = '{0}'", txtID.Text);
                        ds.Clear();
                        koneksi.Open();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtPassword.Text != "" && txtName.Text != "" && txtUsername.Text != "" && txtID.Text != "")
                {

                    query = string.Format("update tbl_user set Password = '{0}', Name = '{1}', level = '{2}' where id = '{3}'", txtPassword.Text, txtName.Text, CBLevel.Text, txtID.Text);
                ...
                if (txtUsername.Text != "")
                {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow kolom in ds.Tables[0].Rows)
                        {
                            ...
                            txtName.Text = kolom["Name"].ToString();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtUsername.Text != "" && txtPassword.Text != "" && txtName.Text != "")
                {
                    ...
                    query = string.Format("insert into tbl_user  values ('{0}','{1}','{2}','{3}','{4}');", txtID.Text, txtUsername.Text, txtPassword.Text, txtName.Text, CBLevel.Text);
